...
test("inclui o núcleo de corrida e conforto WebXR", async () => {
  ...
  assert.match(engine, /`Mão dominante · \$\{this\.audioSettings\.leftHanded\?"Esquerda":"Direita"\}`/);
  ...
  assert.match(engine, /fire=Boolean\(dominant\?\.buttons\[0\]\?\.pressed\)/);
  assert.match(engine, /switchWeapon=Boolean\(dominant\?\.buttons\[5\]\?\.pressed\)/);
  assert.match(engine, /createHandWeapon/);
  assert.match(engine, /Arma modular da mão dominante/);
  assert.match(engine, /private dominantHandAim/);
