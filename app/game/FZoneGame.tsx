...
export function FZoneGame(){
  ...
  return <main className="fz-shell">
    ...
    {race.phase!=="menu"&&<section className="race-hud" aria-live="polite">
      ...
      <div className="speed-hud"><div className="speed-number"><strong>{Math.round(race.speed*7.2)}</strong><span>km/h</span></div>{(race.drafting||race.drifting)&&<div className={`race-technique ${race.drafting?"drafting":""}`}>{race.drafting?"Vácuo ativo + velocidade":"Derrapagem"}</div>}<div className="energy-label"><span>Fluxo de impulso</span><b>{Math.round(race.boost)}%</b></div><div className="energy-track"><div className={`energy-fill ${race.boost<24?"low":""}`} style={{width:`${race.boost}%`}}/></div></div>
