...
import { RIVAL_NAMES,type GameModeId } from "./game-data";
...
const CYAN=0x69f6ff,AMBER=0xffc25e,MAGENTA=0xff3e93,TRACK_WIDTH=28,TOTAL_LAPS=3,SAMPLES=520;
...
export class FZoneEngine{
  ...
  private buildWorld(){
    ...
    this.rig.add(this.camera);this.camera.position.set(0,1.28,.12);this.rig.add(this.ship);this.ship.position.set(0,.02,.62);this.buildCockpit();this.buildBoostStreaks();this.scene.add(this.rig);
  ...
  private addBoostPads(){this.boostPads.forEach((progress,index)=>{const f=frame(this.curve,progress);for(const lane of [-8.5,0,8.5]){const pad=new THREE.Mesh(new THREE.BoxGeometry(3.2,.035,7),new THREE.MeshBasicMaterial({color:index%2?AMBER:CYAN,transparent:true,opacity:.78}));pad.position.copy(f.center).addScaledVector(f.right,lane).addScaledVector(f.up,.03);pad.quaternion.copy(f.quaternion);this.scene.add(pad);}});}
  ...
  configureRace(mode:GameModeId){this.gameMode=mode;this.rivals.forEach(rival=>rival.root.visible=mode!=="arcade");this.emitSnapshot(true);}
  returnToMenu(){this.snapshot.phase="menu";this.speed=0;this.boosting=false;this.lastBoosting=false;this.countdownPanel.visible=false;this.audio.update(0,0,false,false);this.emitSnapshot(true);}
  async enterVR(){void this.audio.start();if(!globalThis.isSecureContext)throw new Error("O modo VR exige uma página HTTPS segura");if(!navigator.xr)throw new Error("A API WebXR não apareceu. Abra no Meta Quest Browser e recarregue a página");const shouldStart=this.snapshot.phase==="menu"||this.snapshot.phase==="finished";const session=await navigator.xr.requestSession("immersive-vr",{optionalFeatures:["local-floor","bounded-floor","hand-tracking"]});await this.renderer.xr.setSession(session);if(shouldStart)this.start();}
  start(){void this.audio.start();if(this.snapshot.phase==="paused"){this.snapshot.phase="racing";this.clock.getDelta();this.emitSnapshot(true);return;}this.distance=0;this.previousProgress=0;this.lateral=0;this.lateralVelocity=0;this.headingOffset=0;this.speed=0;this.boost=100;this.boosting=false;this.lastBoosting=false;this.lap=1;this.lapTime=0;this.bestLap=null;this.elapsed=0;this.drafting=false;this.drifting=false;this.countdownTime=3.05;this.goTime=0;this.countdownLabel="3";this.lastCountdownLabel="3";this.passedPads.clear();this.rivals.forEach((rival,index)=>{const lane=(index-2)*3.8;rival.distance=18+index*14;rival.lane=lane;rival.targetLane=lane;rival.decisionTimer=.55+index*.23;});this.snapshot.phase="countdown";this.drawCountdownPanel("3");this.countdownPanel.visible=true;this.pulse(.18,45);this.emitSnapshot(true);this.clock.getDelta();}
  ...
  private loop(){if(this.destroyed)return;const dt=Math.min(this.clock.getDelta(),.05);if(this.snapshot.phase==="countdown")this.updateCountdown(dt);else if(this.snapshot.phase==="racing")this.updateRace(dt);else{this.audio.update(0,0,false,false);this.updateTransforms(dt);}this.renderer.render(this.scene,this.camera);}
  ...
  private updateRace(dt:number){
    ...
    this.speed=advanceSpeed(this.speed,controls.throttle,controls.brake,this.boosting,dt);this.boost=clamp(this.boost+(this.boosting?-31:8.5)*dt,0,100);if(controls.throttle<.03&&controls.brake<.03&&!this.boosting&&this.speed<.75)this.speed=0;this.audio.update(this.speed,controls.throttle,this.boosting,true);
    ...
    const grip=airborne?.65:magnetic?6.8:controls.brake>.12?1.45:3.25,steeringPower=(20+this.speed*.1)*(airborne?.38:1);
    this.lateralVelocity+=controls.steer*steeringPower*dt;this.lateralVelocity*=Math.exp(-grip*dt);this.lateralVelocity=clamp(this.lateralVelocity,-(controls.brake>.12?21:16),controls.brake>.12?21:16);this.lateral+=this.lateralVelocity*dt;
    ...
    this.distance+=this.speed*dt;const progress=(this.distance/this.trackLength)%1,edge=TRACK_WIDTH/2-1.65;
    ...
    if(progress<this.previousProgress&&this.speed>10){this.bestLap=this.bestLap===null?this.lapTime:Math.min(this.bestLap,this.lapTime);this.lap+=1;this.lapTime=0;this.passedPads.clear();if(this.lap>TOTAL_LAPS){this.lap=TOTAL_LAPS;this.snapshot.phase="finished";this.boosting=false;this.lastBoosting=false;this.audio.update(this.speed,0,false,false);this.speed*=.7;this.showMessage("Prova concluída");}else this.showMessage(`Volta ${this.lap}`);}
  ...
  private updateRivals(dt:number,throttle:number){
    ...
    active.forEach((rival,index)=>{
      rival.distance+=(rival.speed+Math.sin(this.elapsed*.7+index)*5)*dt;rival.decisionTimer-=dt;
  ...
  private emitSnapshot(force=false){if(this.destroyed&&!force)return;const progress=(this.distance/this.trackLength)%1,active=this.activeRivals(),opponents=active.map(rival=>({name:rival.name,progress:((rival.distance/this.trackLength)%1+1)%1,lane:rival.lane,place:1+[this.distance,...active.filter(other=>other!==rival).map(other=>other.distance)].filter(distance=>distance>rival.distance).length,color:`#${rival.color.toString(16).padStart(6,"0")}`,delta:signedTrackDelta(rival.distance,this.distance,this.trackLength)})).sort((a,b)=>a.place-b.place);this.snapshot={...this.snapshot,speed:this.speed,boost:this.boost,boosting:this.boosting,lap:this.lap,totalLaps:TOTAL_LAPS,lapTime:this.lapTime,bestLap:this.bestLap,position:this.position(),racers:active.length+1,opponents,progress,sector:trackSector(progress),airborne:isGap(progress),magnetic:Math.abs(magneticBank(progress))>.1,drafting:this.drafting,drifting:this.drifting,countdown:this.countdownLabel};this.emit({...this.snapshot});}
