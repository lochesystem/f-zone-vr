...
export class FZoneEngine{
  ...
  private async checkVR(){try{this.snapshot.vrSupported=Boolean(navigator.xr&&await navigator.xr.isSessionSupported("immersive-vr"));}catch{this.snapshot.vrSupported=false;}this.snapshot.vrChecked=true;this.emitSnapshot(true);}
  async enterVR(){if(!navigator.xr)throw new Error("WebXR não está disponível neste navegador");const shouldStart=this.snapshot.phase==="menu"||this.snapshot.phase==="finished";const session=await navigator.xr.requestSession("immersive-vr",{optionalFeatures:["local-floor","bounded-floor","hand-tracking"]});await this.renderer.xr.setSession(session);if(shouldStart)this.start();}
