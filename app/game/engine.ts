...
type Rival={root:THREE.Group;distance:number;speed:number;lane:number;targetLane:number;decisionTimer:number;color:number};
...
export class FZoneEngine{
  ...
  private buildWorld(){
    ...
    [MAGENTA,AMBER,0x795cff,0x4cff83,0xff5b3d].forEach((color,index)=>{const root=createRacer(color);root.scale.setScalar(.82);this.scene.add(root);const lane=(index-2)*3.8;this.rivals.push({root,distance:18+index*14,speed:102+index*3.7,lane,targetLane:lane,decisionTimer:.55+index*.23,color});});this.updateTransforms(0);
  }
  private addRails(){
  ...
  private updateRivals(dt:number,throttle:number){
    const laneLimit=TRACK_WIDTH/2-2.3;
  ...
  private emitSnapshot(force=false){if(this.destroyed&&!force)return;const progress=(this.distance/this.trackLength)%1,opponents=this.rivals.map(rival=>({progress:((rival.distance/this.trackLength)%1+1)%1,lane:rival.lane,place:1+[this.distance,...this.rivals.filter(other=>other!==rival).map(other=>other.distance)].filter(distance=>distance>rival.distance).length,color:`#${rival.color.toString(16).padStart(6,"0")}`,delta:signedTrackDelta(rival.distance,this.distance,this.trackLength)})).sort((a,b)=>a.place-b.place);this.snapshot={...this.snapshot,speed:this.speed,boost:this.boost,boosting:this.boosting,lap:this.lap,totalLaps:TOTAL_LAPS,lapTime:this.lapTime,bestLap:this.bestLap,position:this.position(),racers:this.rivals.length+1,opponents,progress,sector:trackSector(progress),airborne:isGap(progress),magnetic:Math.abs(magneticBank(progress))>.1,drafting:this.drafting,drifting:this.drifting,countdown:this.countdownLabel};this.emit({...this.snapshot});}
