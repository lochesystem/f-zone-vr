...
type Rival={root:THREE.Group;distance:number;speed:number;lane:number;targetLane:number;decisionTimer:number};
...
export class FZoneEngine{
  ...
  private buildWorld(){
    ...
    [MAGENTA,AMBER,0x795cff,0x4cff83,0xff5b3d].forEach((color,index)=>{const root=createRacer(color);root.scale.setScalar(.82);this.scene.add(root);const lane=(index-2)*3.8;this.rivals.push({root,distance:18+index*14,speed:102+index*3.7,lane,targetLane:lane,decisionTimer:.55+index*.23});});this.updateTransforms(0);
  }
  private addRails(){
  ...
  private updateRace(dt:number){
    ...
    this.previousProgress=progress;this.checkPads(progress);this.updateRivals(dt);this.resolveRivalCollisions();this.updateTransforms(dt);
  ...
  private updateRivals(dt:number){
    ...
    this.drafting=false;for(const rival of this.rivals){const ahead=signedTrackDelta(rival.distance,this.distance,this.trackLength);if(ahead>5&&ahead<30&&Math.abs(rival.lane-this.lateral)<2.8){this.drafting=true;break;}}
  ...
  private emitSnapshot(force=false){if(this.destroyed&&!force)return;const progress=(this.distance/this.trackLength)%1;this.snapshot={...this.snapshot,speed:this.speed,boost:this.boost,lap:this.lap,totalLaps:TOTAL_LAPS,lapTime:this.lapTime,bestLap:this.bestLap,position:this.position(),racers:this.rivals.length+1,progress,sector:trackSector(progress),airborne:isGap(progress),magnetic:Math.abs(magneticBank(progress))>.1,drafting:this.drafting,drifting:this.drifting,countdown:this.countdownLabel};this.emit({...this.snapshot});}
